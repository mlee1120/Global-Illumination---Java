...
for (int i = 0; i < width; i++) {
  for (int j = 0; j < height; j++) {
    ...
    case 1:
      ...
      S = new float[]{light[0] - intersection[0], light[1] - intersection[1], light[2] - intersection[2]};
      // normalize S
      ...
      // half angle
      H = new float[]{S[0] + N[0], S[1] + N[1], S[2] + N[2]};
      // normalize H
      ...
      dotTemp = H[0] * N[0] + H[1] * N[1] + H[2] * N[2];
      if (dotTemp >= 0) specDot = pow(dotTemp, specExp);
      else specDot = 0.0;
      specular = new float[]{255.0 * specDot * ks, 255.0 * specDot * ks, 255.0 * specDot * ks};
      if (!shadow) c = new float[]{ambient[0] + diffuse[0] + specular[0], ambient[1] + diffuse[1] + specular[1], ambient[2] + diffuse[2] + specular[2]};
...
save("Assignment_3_Phong_Blinn.png");
