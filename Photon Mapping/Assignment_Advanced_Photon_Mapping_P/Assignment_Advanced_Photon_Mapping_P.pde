...
void setup() {
  // canvas size
  size(600, 600, P3D);
  camera(275, -275, -477, 275, -275, 0.0, 0, 1, 0);
  //size(900, 900);
...
void draw() {
  if (toDraw) {
    ...
    stroke(255);
    beginShape(POINTS);
    for (int i = 0; i < photons2.size(); i++) {
      vertex(photons2.get(i)[0], -photons2.get(i)[1], photons2.get(i)[2]);
    }
    endShape();
    //save("Assignment_Advanced_Photon_Mapping_10000_50p.png");
    toDraw = false;
  }
}
...
float[] calculate_color(float[] origin, Primitive p, float[] intersection, Object[] shadow) {
  ...
  final_color[0] = ph * 255;
  final_color[1] = ph * 255;
  final_color[2] = ph * 255;
  return final_color;
}
...
void scatter() {
  ...
  for (int i = 0; i < photon; i++) {
    ...
    theta = mod(random(0.0, 100.0), 1.0);
    //theta = random(0.0, 1.0);
    phi = mod(random(0.0, 100.0), 1.0);
    //phi = random(0.0, 1.0);
    do {
