  ...
  if (toDraw) {
    ...
    for (int i = 0; i < width; i++) {
      for (int j = 0; j < height; j++) {
        ...
        if (rgbi[0] / supersample > max_color) max_color = rgbi[0] / supersample;
        if (rgbi[1] / supersample > max_color) max_color = rgbi[1] / supersample;
        if (rgbi[2] / supersample > max_color) max_color = rgbi[2] / supersample;
        if (rgbi[3] / supersample > max_intensity) max_intensity = rgbi[3] / supersample;
      }
    }
    //max_color = 255;
    ...
    for (int i = 0; i < width; i++) {
      for (int j = 0; j < height; j++) {
        if (buffer.get(i).get(j)[0] / supersample == 255.0 && buffer.get(i).get(j)[1] / supersample == 255.0 && buffer.get(i).get(j)[2] / supersample == 255.0) set(i, j, color(255.0, 255.0, 255.0));
        //else set(i, j, color(buffer.get(i).get(j)[0] / supersample / max_color * 255 * buffer.get(i).get(j)[3] / supersample / max_intensity, buffer.get(i).get(j)[1] / supersample / max_color * 255 * buffer.get(i).get(j)[3] / supersample / max_intensity, buffer.get(i).get(j)[2] / supersample / max_color * 255 * buffer.get(i).get(j)[3] / supersample / max_intensity));
        else {
          rgbs = buffer.get(i).get(j)[3] / supersample / scale;
          if (rgbs > 1.1) rgbs = 1.1;
          set(i, j, color(buffer.get(i).get(j)[0] / supersample * rgbs, buffer.get(i).get(j)[1] / supersample * rgbs, buffer.get(i).get(j)[2] / supersample * rgbs));
        }
      }
    }
    end = millis();
    ...
    //save("Assignment_Advanced_Photon_Mapping_Irradiance_3000000.png");
    toDraw = false;
  }
}
...
float[] calculate_color(float[] origin, Primitive p, float[] intersection, Object[] shadow) {
  ...
  float[] final_color = new float[]{0.0, 0.0, 0.0, 0.0};
  ...
  if (p.type.equals("background")) {
    final_color[0] = p.rgb[0];
    final_color[1] = p.rgb[1];
    final_color[2] = p.rgb[2];
...
float[] reflect(float[] origin, float[] ray) {
  ...
  float[] reflected_color = new float[]{0.0, 0.0, 0.0, 0.0};
...
void scatter() {
  ...
  for (int i = 0; i < photon; i++) {
    ...
    temp2[0] = origin[0] - mod(origin[0], 0.001);
    temp2[1] = origin[1] - mod(origin[1], 0.001);
    temp2[2] = origin[2] - mod(origin[2], 0.001);
...
int searchTree(Node current, float[] intersection) {
  ...
  if (current.name.equals("yes")) {
    if (calculate_distance(current.position, intersection) < distance) {
      result += photons.get(current.position[0] + " " + current.position[1] + " " + current.position[2]);
      for (int i = 0; i < result; i++) {
        maxHeap.add(1.0 / pow(calculate_distance(current.position, intersection), 2.0));
      }
    }
