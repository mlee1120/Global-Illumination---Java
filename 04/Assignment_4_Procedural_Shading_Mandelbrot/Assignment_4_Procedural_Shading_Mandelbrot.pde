...
void draw() {
  if (toDraw) {
    ...
    //save("Assignment_4_Procedural_Shading_Mandelbrot.png");
    toDraw = false;
  }
}
...
void calculate_color() {
  ...
  case 3:
    ...
    // modify u and v for generating Mandelbrot texture
    u = (u - 0.5) * 3.0;
    v = (v - 0.5) * 3.0;

    // Mandelbrot set: Z0 = Z0 * Z0 + C (n iterations) is |Z0| <= 2 => belong to the set
    float[] Z0 = new float[]{u, v};
    float[] C = new float[]{-0.75, 0.0};

    // belong to Mandelbrot set or not
    ...
    // 100 iterations
    while (max < 100) {
      temp = Z0[0] * Z0[0] - Z0[1] * Z0[1];
      Z0[1] = 2 * Z0[0] * Z0[1] + C[1];
      Z0[0] = temp + C[0];
      max++;
    }
    if (sqrt(Z0[0] * Z0[0] + Z0[1] * Z0[1]) <= 2) mandelbrot = true;

    if (mandelbrot) ambient = new float[]{0.0 * ka, 198.0 * ka, 99.0 * ka};
    else ambient = new float[]{255.0 * ka, 228.0 * ka, 108.0 * ka};
    ;
    c[0] += ambient[0];
    c[1] += ambient[1];
    c[2] += ambient[2];
    ...
    for (int k = 0; k < lights.size(); k++) {
      ...
      dotTemp = N[0] * S[0] + N[1] * S[1] + N[2] * S[2];
      if (dotTemp >= 0) {
        if(mandelbrot) diffuse = new float[]{0.0 * dotTemp * kd, 198.0 * dotTemp * kd, 99.0 * dotTemp * kd};
        else diffuse = new float[]{255.0 * dotTemp * kd, 228.0 * dotTemp * kd, 108.0 * dotTemp * kd};
      } else diffuse = new float[]{0.0, 0.0, 0.0};
      dotTemp = R[0] * V[0] + R[1] * V[1] + R[2] * V[2];
      if (dotTemp >= 0) specDot = pow(dotTemp, specExp);
      else specDot = 0.0;
