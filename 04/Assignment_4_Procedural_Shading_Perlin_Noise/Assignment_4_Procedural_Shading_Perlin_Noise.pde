...
void draw() {
  if (toDraw) {
    ...
    //save("Assignment_4_Procedural_Shading_Perlin_Noise.png");
    toDraw = false;
  }
}
...
void calculate_color() {
  ...
  case 3:
    ...
    // calculate 2D perlin noise based on uv 
    ...
    ambient = new float[]{255.0 * ka * noise, 228.0 * ka * noise, 108.0 * ka * noise};
    c[0] += ambient[0];
    c[1] += ambient[1];
    c[2] += ambient[2];
    ...
    for (int k = 0; k < lights.size(); k++) {
      ...
      dotTemp = N[0] * S[0] + N[1] * S[1] + N[2] * S[2];
      if (dotTemp >= 0) diffuse = new float[]{255.0 * dotTemp * kd * noise, 228.0 * dotTemp * kd * noise, 108.0 * dotTemp * kd * noise};
      else diffuse = new float[]{0.0, 0.0, 0.0};
      dotTemp = R[0] * V[0] + R[1] * V[1] + R[2] * V[2];
      if (dotTemp >= 0) specDot = pow(dotTemp, specExp);
      else specDot = 0.0;
