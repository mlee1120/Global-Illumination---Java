...
void draw() {
  if (toDraw) {
    ...
    //save("Assignment_4_Procedural_Shading_Checkerboard_Three.png");
    toDraw = false;
  }
}
...
void calculate_color() {
  ...
  case 3:
    ...
    float dx = procedural / 400.0;
    float dy = procedural / 500.0;
    if ((ceil(u / dx) + ceil(v / dy)) % 3 == 0) ambient = new float[]{200.0 * ka, 0.0 * ka, 0.0 * ka};
    else if ((ceil(u / dx) + ceil(v / dy)) % 2 == 0) ambient = new float[]{0.0 * ka, 200.0 * ka, 0.0 * ka};
    else ambient = new float[]{0.0 * ka, 0.0 * ka, 200.0 * ka};
    ...
    for (int k = 0; k < lights.size(); k++) {
      ...
      dotTemp = N[0] * S[0] + N[1] * S[1] + N[2] * S[2];
      if (dotTemp >= 0) {
        if ((ceil(u / dx) + ceil(v / dy)) % 3 == 0) diffuse = new float[]{200.0 * dotTemp * kd, 0.0 * dotTemp * kd, 0.0 * dotTemp * kd};
        else if ((ceil(u / dx) + ceil(v / dy)) % 2 == 0) diffuse = new float[]{0.0 * dotTemp * kd, 200.0 * dotTemp * kd, 0.0 * dotTemp * kd};
        else diffuse = new float[]{0.0 * dotTemp * kd, 0.0 * dotTemp * kd, 200.0 * dotTemp * kd};
      } else diffuse = new float[]{0.0, 0.0, 0.0};
      dotTemp = R[0] * V[0] + R[1] * V[1] + R[2] * V[2];
      if (dotTemp >= 0) specDot = pow(dotTemp, specExp);
      else specDot = 0.0;
