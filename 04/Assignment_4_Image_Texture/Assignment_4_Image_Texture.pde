...
void setup() {
  ...
  img3 = loadImage("pokemon_rowlet.png");
}
...
void draw() {
  if (toDraw) {
    ...
    //save("Assignment_4_Image_Texture_II.png");
    toDraw = false;
  }
}
...
void calculate_color() {
  ...
  case 1:
    ...
    if (inter.get(inter.size() - 1)[0] == -55 && inter.get(inter.size() - 1)[1] == 160 && inter.get(inter.size() - 1)[2] == 100) {
      u = 0.0;
      v = 0.0;
    } else if (inter.get(inter.size() - 1)[0] == -55 && inter.get(inter.size() - 1)[1] == 40 && inter.get(inter.size() - 1)[2] == 100) {
      u = 0.0;
      v = 1.0;
    } else {
      float[] temp = new float[]{inter.get(inter.size() - 1)[0] - (-55.0), 0.0, inter.get(inter.size() - 1)[2] - 100.0};
      temp = normalize(temp);
      dotTemp = temp[2];
      if (temp[0] > 0) u = acos(dotTemp) / 2.0 / PI;
      else if (temp[0] < 0) u = (2.0 * PI - acos(dotTemp)) / 2.0 / PI;
      else {
        if (temp[2] > 0) u = 0.0;
        else u = 0.5;
      }
    ...
    pixel_x = round(u * img1.width);
    pixel_z = round(v * img1.height);

    // extract the pixel rgb values
    r = red(img1.pixels[pixel_x + pixel_z * img1.width]);
    g = green(img1.pixels[pixel_x + pixel_z * img1.width]);
    b = blue(img1.pixels[pixel_x + pixel_z * img1.width]);

    ambient = new float[]{r * ka, g * ka, b * ka};
    ...
    for (int k = 0; k < lights.size(); k++) {
      ...
      if (dotTemp >= 0) diffuse = new float[]{r * dotTemp * kd, g * dotTemp * kd, b * dotTemp * kd};
  ...
  case 2:
    ...
    if (inter.get(inter.size() - 1)[0] == 15 && inter.get(inter.size() - 1)[1] == 105 && inter.get(inter.size() - 1)[2] == 5) {
      u = 0.0;
      v = 0.0;
    } else if (inter.get(inter.size() - 1)[0] == 15 && inter.get(inter.size() - 1)[1] == 15 && inter.get(inter.size() - 1)[2] == 5) {
      u = 0.0;
      v = 1.0;
    } else {
      float[] temp = new float[]{inter.get(inter.size() - 1)[0] - 15.0, 0.0, inter.get(inter.size() - 1)[2] - 5.0};
      temp = normalize(temp);
      dotTemp = temp[2];
      if (temp[0] > 0) u = acos(dotTemp) / 2.0 / PI;
      else if (temp[0] < 0) u = (2.0 * PI - acos(dotTemp)) / 2.0 / PI;
    ...
    // locate the pixel in the image
    pixel_x = round(u * img2.width);
    pixel_z = round(v * img2.height);

    // extract the pixel rgb values
    r = red(img2.pixels[pixel_x + pixel_z * img2.width]);
    g = green(img2.pixels[pixel_x + pixel_z * img2.width]);
    b = blue(img2.pixels[pixel_x + pixel_z * img2.width]);
  ...
  case 3:
    ...
    // locate the pixel in the image
    pixel_x = round(u * img3.width);
    pixel_z = round(v * img3.height);

    // extract the pixel rgb values
    r = red(img3.pixels[pixel_x + pixel_z * img3.width]);
    g = green(img3.pixels[pixel_x + pixel_z * img3.width]);
    b = blue(img3.pixels[pixel_x + pixel_z * img3.width]);
