...
void draw() {
  if (toDraw) {
    ...
    //save("Assignment_4_Procedural_Shading_II.png");
    toDraw = false;
  }
}
...
void calculate_color() {
  ...
  case 1:
    ...
    dx = procedural / 400.0 * 0.8;
    dz = procedural / 500.0 * 0.8;
    if ((ceil(u / dx) + ceil(v / dz)) % 2 == 0) ambient = new float[]{0.0 * ka, 255.0 * ka, 0.0 * ka};
    else ambient = new float[]{0.0 * ka, 0.0 * ka, 255.0 * ka};
    ...
    for (int k = 0; k < lights.size(); k++) {
      ...
      if (dotTemp >= 0) {
        if ((ceil(u / dx) + ceil(v / dz)) % 2 == 0) diffuse = new float[]{0.0 * dotTemp * kd, 255.0 * dotTemp * kd, 0.0 * dotTemp * kd};
        else diffuse = new float[]{0.0 * dotTemp * kd, 0.0 * dotTemp * kd, 255.0 * dotTemp * kd};
  ...
  case 2:
    ...
    // checkerboard size in uv
    dx = procedural / 400.0 * 0.5;
    dz = procedural / 500.0 * 0.5;
    if ((ceil(u / dx) + ceil(v / dz)) % 2 == 0) ambient = new float[]{0.0 * ka, 200.0 * ka, 200.0 * ka};
    else ambient = new float[]{255.0 * ka, 0.0 * ka, 255.0 * ka};
    ...
    for (int k = 0; k < lights.size(); k++) {
      ...
      dotTemp = N[0] * S[0] + N[1] * S[1] + N[2] * S[2];
      if (dotTemp >= 0) {
        if ((ceil(u / dx) + ceil(v / dz)) % 2 == 0) diffuse = new float[]{0.0 * dotTemp * kd, 200.0 * dotTemp * kd, 200.0 * dotTemp * kd};
        else diffuse = new float[]{255.0 * dotTemp * kd, 0.0 * dotTemp * kd, 255.0 * dotTemp * kd};
      } else diffuse = new float[]{0.0, 0.0, 0.0};
      dotTemp = R[0] * V[0] + R[1] * V[1] + R[2] * V[2];
      if (dotTemp >= 0) specDot = pow(dotTemp, specExp);
      else specDot = 0.0;
