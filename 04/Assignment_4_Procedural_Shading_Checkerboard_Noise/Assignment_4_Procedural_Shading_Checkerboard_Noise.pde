...
void draw() {
  if (toDraw) {
    ...
    //save("Assignment_4_Procedural_Shading_Checkerboard_Noise.png");
    toDraw = false;
  }
}
...
void calculate_color() {
  ...
  case 3:
    ...
    noiseDetail(9, 0.75);
    float frequency = 3;
    float noise = noise(u * frequency, v * frequency);
    
    if ((ceil(u / dx) + ceil(v / dy)) % 2 == 0) ambient = new float[]{255.0 * ka * noise, 255.0 * ka * noise, 0.0 * ka * noise};
    else ambient = new float[]{255.0 * ka * noise, 0.0 * ka * noise, 0.0 * ka * noise};
    ...
    for (int k = 0; k < lights.size(); k++) {
      ...
      dotTemp = N[0] * S[0] + N[1] * S[1] + N[2] * S[2];
      if (dotTemp >= 0) {
        if ((ceil(u / dx) + ceil(v / dy)) % 2 == 0) diffuse = new float[]{255.0 * dotTemp * kd * noise, 255.0 * dotTemp * kd * noise, 0.0 * dotTemp * kd * noise};
        else diffuse = new float[]{255.0 * dotTemp * kd * noise, 0.0 * dotTemp * kd * noise, 0.0 * dotTemp * kd * noise};
      } else diffuse = new float[]{0.0, 0.0, 0.0};
      dotTemp = R[0] * V[0] + R[1] * V[1] + R[2] * V[2];
      if (dotTemp >= 0) specDot = pow(dotTemp, specExp);
      else specDot = 0.0;
