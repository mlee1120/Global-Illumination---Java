...
void draw() {
  if (toDraw) {
    ...
    //save("Assignment_5_1000_Rays.png");
    toDraw = false;
  }
}
...
void calculate_color() {
  ...
  case 2:
    ...
    for (int i = 0; i < lights.size(); i++) {
      ...
      float[] reflection = reflect(max_depth, 0.3, inter.get(inter.size() - 1), RR, N);
      c[0] += reflection[0];
      c[1] += reflection[1];
      c[2] += reflection[2];
      colors.add(c);
...
float[] reflect(int depth, float kr, float[] point, float[] direction, float[] norm) {
  ...
  if (depth != 0) {
    ...
    for (int k = 0; k < rays; k++) {
      ...
      float[] new_direction = new float[3];
      Quaternion rand, qpoint, qnew;
      float u0 = random(0.0, 1.0), u1 = random(0.0, 1.0), u2 = random(0.0, 1.0);
      float Kd = 0.5, Ks = 0.3, SpecExp = 30.0;
      // diffuse sample
      if (u0 <= Kd) {
        theta = acos(sqrt(u1));
        phi = 2 * PI * u2;
        axis = normalize(crossProduct(norm, direction));
        rand = new Quaternion(cos(theta / 2.0), axis[0] * sin(theta / 2.0), axis[1] * sin(theta / 2.0), axis[2] * sin(theta / 2.0));
        qpoint = new Quaternion(0.0, norm[0], norm[1], norm[2]);
        qnew = rand.product(qpoint).product(rand.inverse());
        new_direction[0] = qnew.x;
        new_direction[1] = qnew.y;
        new_direction[2] = qnew.z;
        axis = normalize(norm);
        rand = new Quaternion(cos(phi / 2.0), axis[0] * sin(phi / 2.0), axis[1] * sin(phi / 2.0), axis[2] * sin(phi / 2.0));
        qpoint = new Quaternion(0.0, new_direction[0], new_direction[1], new_direction[2]);
        qnew = rand.product(qpoint).product(rand.inverse());
        new_direction[0] = qnew.x;
        new_direction[1] = qnew.y;
        new_direction[2] = qnew.z;
      ...
      // specular sample
        ...
        alpha = acos(pow(u1, 1 / (SpecExp + 1.0)));
        phi = 2 * PI * u2;
        axis = normalize(crossProduct(norm, direction));
        rand = new Quaternion(cos(alpha / 2.0), axis[0] * sin(alpha / 2.0), axis[1] * sin(alpha / 2.0), axis[2] * sin(alpha / 2.0));
        qpoint = new Quaternion(0.0, direction[0], direction[1], direction[2]);
        qnew = rand.product(qpoint).product(rand.inverse());
        new_direction[0] = qnew.x;
        new_direction[1] = qnew.y;
        new_direction[2] = qnew.z;
      ...
      // check sphere1
      A = new_direction[0] * new_direction[0] + new_direction[1] * new_direction[1] + new_direction[2] * new_direction[2];
      B = 2 * (point[0] + 55) * new_direction[0] + 2 * (point[1] - 100) * new_direction[1] + 2 * (point[2] - 100) * new_direction[2];
      ...
      if (D >= 0) {
        ...
        if (coefficient > 0.001) {
          choiceR = 1;
          intersectionR[0] = point[0] + new_direction[0] * coefficient;
          intersectionR[1] = point[1] + new_direction[1] * coefficient;
          intersectionR[2] = point[2] + new_direction[2] * coefficient;
      ...
      if (choiceR == 0) {
        A = new_direction[0] * new_direction[0] + new_direction[1] * new_direction[1] + new_direction[2] * new_direction[2];
        B = 2 * (point[0] - 15) * new_direction[0] + 2 * (point[1] - 60) * new_direction[1] + 2 * (point[2] - 5) * new_direction[2];
      ...
      // check floor
      if (choiceR == 0) {
        coefficient = -point[1] / new_direction[1];
        if (coefficient >= 0) {
          intersectionR[0] = point[0] + new_direction[0] * coefficient;
          intersectionR[1] = point[1] + new_direction[1] * coefficient;
          intersectionR[2] = point[2] + new_direction[2] * coefficient;
      ...
      case 1:
        ...
        cr = reflect(depth, 0.0, intersectionR, RR, N);
        c[0] += cr[0];
        c[1] += cr[1];
        c[2] += cr[2];
        break;
      case 2:
        ...
        dotTemp = V[0] * N[0] + V[1] * N[1] + V[2] * N[2];
        RR = new float[]{-V[0] + 2 * N[0] * dotTemp, -V[1] + 2 * N[1] * dotTemp, -V[2] + 2 * N[2] * dotTemp};
        cr = reflect(depth, 0.3, intersectionR, RR, N);
        c[0] += cr[0];
        c[1] += cr[1];
        c[2] += cr[2];
        break;
      ...
      alpha = acos(direction[0] * new_direction[0] + direction[1] * new_direction[1] + direction[2] * new_direction[2]);
      float BRDF = Kd / PI + Ks * (SpecExp + 2) / 2 / PI * pow(cos(alpha), SpecExp);
      if (u0 <= Kd + Ks) {
        result[0] += kr * c[0] * BRDF;
        result[1] += kr * c[1] * BRDF;
        result[2] += kr * c[2] * BRDF;
        total += BRDF;
      }
    }
  }
